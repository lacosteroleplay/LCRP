...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	/* if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) {
			format(string, sizeof (string), "This account (%s) is not registered.\nPlease input your password below to register your account:\nYour password must be longer than 5 characters.", ReturnName(playerid));
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "LC:RP - Registration", string, "Register", "Cancel");
			return 1;
		}
		...
		if(!IsValidPassword(inputtext))
		{
			...
			format(string, sizeof (string), "This account (%s) is not registered.\nPlease input your password below to register your account:", ReturnName(playerid));
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "LC:RP - Registration", string, "Register", "Cancel");
			return 1;
		}
	...
	if(dialogid == DIALOG_MY_BISNIS)
	{
		...
		ShowPlayerDialog(playerid, BISNIS_INFO, DIALOG_STYLE_LIST, "{FF0000}LC:RP {0000FF}Bisnis", "Show Information\nTrack Bisnis", "Select", "Cancel");
		return 1;
	}
		...
		if(response)
		{
			if (Player_OwnsBisnis(playerid, bid)) {
				switch(listitem)
				{
					...
					case 5: {
						new str[512];
						format(str, sizeof(str), "%s\n%s\n%s\n%s\n%s", bData[bid][bWorker1], bData[bid][bWorker2], bData[bid][bWorker3], bData[bid][bWorker4], bData[bid][bWorker5]);
						ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, "Bisnis Worker", str, "Select", "Close");
					}
				}
			} else if (Player_BisnisWorker(playerid, pData[playerid][pWorkerBiz])) {
				switch (listitem) {
					case 0: {
						new mstr[248], lstr[512];
						format(mstr,sizeof(mstr),"Bisnis ID %d", bid);
		...
		if((bizid = pData[playerid][pInBiz]) != -1 && response)
		{
			...
			else if(bData[bizid][bType] == 2)
			{
				switch(listitem)
				{
					...
					{
						GivePlayerMoneyEx(playerid, -price);
						pData[playerid][pDice] = 1;
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli sebuah dice seharga %s.", ReturnName(playerid), FormatMoney(price));
						bData[bizid][bProd]--;
						bData[bizid][bMoney] += Server_Percent(price);
						Server_AddPercent(price);
						Bisnis_Save(bizid);
					...
					{
						GivePlayerMoneyEx(playerid, -price);
						pData[playerid][pTollCard] = 50;
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli sebuah toll card seharga %s.", ReturnName(playerid), FormatMoney(price));
						bData[bizid][bProd]--;
						bData[bizid][bMoney] += Server_Percent(price);
						Server_AddPercent(price);
						Bisnis_Save(bizid);
			...
			else if(bData[bizid][bType] == 4)
			{
				switch(listitem)
				{
					...
					case 8:
					{
						...
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli boombox seharga %s.", ReturnName(playerid), FormatMoney(price));
						bData[bizid][bProd]--;
						bData[bizid][bMoney] += Server_Percent(price);
						Server_AddPercent(price);
						Bisnis_Save(bizid);
					...
					case 9:
					{
						if (pData[playerid][pTrawl] >= 200) return Error(playerid,"Maximal trawl adalah 200.");
						GivePlayerMoneyEx(playerid, -price);
						pData[playerid][pTrawl] += 20;
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "** %s telah membeli Trawl sebanyak 20 dengan harga %s.", ReturnName(playerid), FormatMoney(price));
						bData[bizid][bProd]--;
						bData[bizid][bMoney] += Server_Percent(price);
						Server_AddPercent(price);
						Bisnis_Save(bizid);
	...
	/* new selectedWork;
	if (dialogid == BISNIS_WORKER) {
		if (response) {
			new id = pData[playerid][pInBiz];
			switch (listitem) {
				case 0: {
					if (strcmp(bData[id][bWorker1], "Vacant", true)) {
						selectedWork = 1;
						ShowPlayerDialog(playerid, BISNIS_WORKEROPT, DIALOG_STYLE_LIST, "Employee Option", "Fired", "Select", "Back");
					} else {
						selectedWork = 1;
						ShowPlayerDialog(playerid, BISNIS_WORKEROPT, DIALOG_STYLE_LIST, "Employee Option", "Hired", "Select", "Back");
					}
				}
				case 1: {
					if (strcmp(bData[id][bWorker2], "Vacant", true)) {
						selectedWork = 2;
						ShowPlayerDialog(playerid, BISNIS_WORKEROPT, DIALOG_STYLE_LIST, "Employee Option", "Fired", "Select", "Back");
					} else {
						selectedWork = 2;
				...
				case 2: {
					if (strcmp(bData[id][bWorker3], "Vacant", true)) {
						selectedWork = 3;
						ShowPlayerDialog(playerid, BISNIS_WORKEROPT, DIALOG_STYLE_LIST, "Employee Option", "Fired", "Select", "Back");
					} else {
						selectedWork = 3;
				...
				case 3: {
					if (strcmp(bData[id][bWorker4], "Vacant", true)) {
						selectedWork = 4;
						ShowPlayerDialog(playerid, BISNIS_WORKEROPT, DIALOG_STYLE_LIST, "Employee Option", "Fired", "Select", "Back");
					} else {
						selectedWork = 4;
				...
				case 4: {
					if (strcmp(bData[id][bWorker5], "Vacant", true)) {
						selectedWork = 5;
						ShowPlayerDialog(playerid, BISNIS_WORKEROPT, DIALOG_STYLE_LIST, "Employee Option", "Fired", "Select", "Back");
					} else {
						selectedWork = 5;
	...
	if (dialogid == BISNIS_WORKEROPT) {
		if (response) {
			new id = pData[playerid][pInBiz];
			if (!strcmp(Getselected_Worker(selectedWork, id), "Vacant", true)) {
				switch (listitem) {
					case 0: {
						if (bData[pData[playerid][pInBiz]][bWork] >= 5) return Error(playerid,"Your employee is full!");
						ShowPlayerDialog(playerid, BISNIS_HIRED, DIALOG_STYLE_INPUT, "Business Hire Employee", "Please input playerid or name in the below:", "Hire", "Cancel");
					}
				}
			} else {
				switch (listitem) {
					case 0: {
						ShowPlayerDialog(playerid, BISNIS_FIRED, DIALOG_STYLE_INPUT, "Business Fire Employee", "Please input playerid or name in the below:", "Fire", "Cancel");
					}
				}
			}
		}
	...
	if (dialogid == BISNIS_HIRED) {
		if (response) {
			new otherid = strval(inputtext);
			if (!IsPlayerConnected(otherid)) return Error(playerid,"Invalid playerid!");
			if (!NearPlayer(playerid, otherid, 4.0)) return Error(playerid, "You are not near with player!");
			if (pData[playerid][pInBiz] == -1) return Error(playerid,"You must in your business!");

			pData[otherid][pWorkerBiz] = pData[playerid][pInBiz];
			new id = pData[playerid][pInBiz];
			Setbiz_Worker(id, selectedWork, otherid);
			selectedWork = 0;
			bData[pData[playerid][pInBiz]][bWork]++;
			Info(playerid,"You has been hired %s to your business.", pData[otherid][pName]);
			Info(otherid,"%s has hired your to business.", pData[playerid][pName]);
			Bisnis_Save(pData[playerid][pInBiz]);
		}
		return 1;
	}
	if (dialogid == BISNIS_FIRED) {
		if (response) {
			new otherid = strval(inputtext), id = pData[playerid][pInBiz];

			if (!IsPlayerConnected(otherid)) return Error(playerid,"Invalid playerid!");
			if (!NearPlayer(playerid, otherid, 4.0)) return Error(playerid, "You are not near with player!");
			if (pData[playerid][pInBiz] == -1) return Error(playerid,"You must in your business!");

			pData[otherid][pWorkerBiz] = -1;
			Resetbiz_Worker(selectedWork, id);
			bData[id][bWork]--;
			selectedWork = 0;
			Info(playerid,"You has been fired %s from your business.",pData[otherid][pName]);
			Info(otherid,"%s has fired you from business.",pData[playerid][pName]);
			Bisnis_Save(id);
		}
		return 1;
	} */
	...
	if(dialogid == DIALOG_MY_HOUSES)
	{
		...
		ShowPlayerDialog(playerid, HOUSE_INFO, DIALOG_STYLE_LIST, "{FF0000}LC:RP {0000FF}Houses", "Show Information\nTrack House", "Select", "Cancel");
		return 1;
	}
			...
			switch(listitem)
			{
				case 0: //slot 1
				{
					...
					else
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"LC:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
				}
				case 1: //slot 2
				{
					...
					else
					{
						...
						pToys[playerid][pData[playerid][toySelected]][toy_rx], pToys[playerid][pData[playerid][toySelected]][toy_ry], pToys[playerid][pData[playerid][toySelected]][toy_rz]);
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"LC:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
						//ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"LC:RP "WHITE_E"Player Toys", ""dot"Edit Toy Position\n"dot"Change Bone\n"dot""GREY_E"Remove Toy\n"dot"Share Toy Pos", "Select", "Cancel");
					}
				}
				case 2: //slot 3
				{
	...
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			...
			{
				new str[3500];
				strcat(str, ""LG_E"PLAYER: /help /drag /undrag /pay /stats /afk /items /frisk /use /give /idcard /drivelic\n");
				strcat(str, ""LB_E"PLAYER: /weapon /settings /mask /helmet /death /accept /deny /buy /ask /phone /quizanswer /paytoll\n");
			...
			case 3:
			{
				...
				strcat(line3, ""LB_E"8. Nelayan\n");
				strcat(line3, ""WHITE_E"/throwtrawl, /picktrawl\n\n");
			...
			{
				new str[3500];
				strcat(str, ""LG_E"BISNIS: /buy /bm /lockbisnis /unlockbisnis /mybis /inviteworker /uninviteworker\n");
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"Chat", str, "Close", "");
			}
			case 7:
			{
	...
	if(dialogid == DIALOG_GPS)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 8:
				{
					SetPlayerRaceCheckpoint(playerid,1, 1315.92, -1423.46, 13.52, 0.0, 0.0, 0.0, 3.5);
					Info(playerid, "GPS active! follow the checkpoint.");
				}
				case 9:
				{
					ShowPlayerDialog(playerid, DIALOG_GPS_JOB, DIALOG_STYLE_LIST, "GPS JOB", "Taxi\nMechanic\nLumber Jack\nTrucker\nMiner\nProduction\nFarmer\nNelayan\nMilk (Side Job)\nSweeper(Side Job)\nBus(Side Job)\nTrashmaster (Side Job)\nMower (Side Job)", "Select", "Close");
				}
			}
		}
	}
	if(dialogid == DIALOG_GPS_JOB)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 7:
				{
					SetPlayerRaceCheckpoint(playerid, 1, 154.19, -1946.61, 5.38, 0.0, 0.0, 0.0, 3.5); //Nelayan
					Info(playerid, "GPS Active! Follow the checkpoint!");
				}
				case 8:
				{
				    SetPlayerRaceCheckpoint(playerid, 1, -670.1496,969.9528,12.1328,0.0,0.0,0.0,3.5); //Milk
				    Info(playerid, "GPS Active! Follow the checkpoint.");
				}
				case 9:
				{
				...
				case 11:
				{
					SetPlayerRaceCheckpoint(playerid, 1, 2432.553710, -2115.611328, 13.546875, 0.0, 0.0, 0.0, 3.5); //Trashmaster
					Info(playerid,"GPS Active! Follow the checkpoint.");
				}
				case 12:
				{
					SetPlayerRaceCheckpoint(playerid, 1, 545.79, -1781.32, 5.85, 0.0, 0.0, 0.0, 3.5); //Mower
					Info(playerid,"GPS Active! Follow the checkpoint.");
				}
			}
		}
	    ...
	    if (response) {
	        switch (listitem) {
	            ...
	            case 2: {
	                ...
	                ShowPlayerDialog(playerid,DIALOG_SMS,DIALOG_STYLE_INPUT,"SMS",""WHITE_E"Please input a Phone Number: (input below)","Input","Close");
	            }
	        }
	    }
	    return 1;
	...
	if (dialogid == DIALOG_SMS) {
	    if (response) {
			...
			if (isnull(inputtext) || !IsNumeric(inputtext))
			    return ShowPlayerDialog(playerid,DIALOG_SMS,DIALOG_STYLE_INPUT,"SMS","Invalid format!!!\n"WHITE_E"Please input a Phone Number: (input below)","Input","Close");

			pData[playerid][pContact] = ph;
			ShowPlayerDialog(playerid,DIALOG_SMS_TEXT,DIALOG_STYLE_INPUT,"SMS - Text",""WHITE_E"Please input a message: (input below)","Send","Close");
	    }
	    return 1;
	}
	if (dialogid == DIALOG_SMS_TEXT) {
	    if (response) {
	        foreach (new i : Player) if (pData[i][pPhone] == pData[playerid][pContact]) {
	            if(i == INVALID_PLAYER_ID || !IsPlayerConnected(i)) return Error(playerid, "This number is not actived!");
            	SendClientMessageEx(playerid, COLOR_YELLOW, "[SMS to %d]"WHITE_E" %s", pData[playerid][pContact], inputtext);
				SendClientMessageEx(i, COLOR_YELLOW, "[SMS from %d]"WHITE_E" %s", pData[playerid][pPhone], inputtext);
				PlayerPlaySound(i, 6003, 0,0,0);
				pData[i][pSMS] = pData[playerid][pPhone];
	...
	if (dialogid == DIALOG_CONTACTS_LIST) {
	    if (response)
		{
		    if (!listitem) {
		        ShowPlayerDialog(playerid, DIALOG_NEW_CONTACT, DIALOG_STYLE_INPUT, "New Contact", "Please enter the name of the contact below:", "Submit", "Back");
		    }
		    else {
			    pData[playerid][pContact] = ListedContacts[playerid][listitem - 1];

		        ShowPlayerDialog(playerid, DIALOG_CONTACT_INFO, DIALOG_STYLE_LIST, ContactData[playerid][pData[playerid][pContact]][contactName], "Delete Contact", "Select", "Back");
		    }
		}
	...
	if (dialogid == DIALOG_CONTACT_INFO) {
        if (response)
		{
		    new
			...
			switch (listitem)
			{
			    case 0:
			    {
			        ...
			        Servers(playerid, "You have deleted \"%s\" from your contacts.", ContactData[playerid][id][contactName]);
	...
	if (dialogid == DIALOG_NEW_CONTACT) {
     	if (response)
		{
		    new str[128];
		    if (isnull(inputtext))
				return ShowPlayerDialog(playerid, DIALOG_NEW_CONTACT, DIALOG_STYLE_INPUT, "New Contact", "Error: Please enter a contact name.\n\nPlease enter the name of the contact below:", "Submit", "Back");

		    if (strlen(inputtext) > 32)
		        return ShowPlayerDialog(playerid, DIALOG_NEW_CONTACT, DIALOG_STYLE_INPUT, "New Contact", "Error: The contact name can't exceed 32 characters.\n\nPlease enter the name of the contact below:", "Submit", "Back");

			strpack(pData[playerid][pEditingItems], inputtext, 32);
			
			format(str,sizeof(str),"Contact Name: %s\n\nPlease enter the phone number for this contact:",inputtext);

		    ShowPlayerDialog(playerid, DIALOG_ENTER_NUMBER, DIALOG_STYLE_INPUT, "Contact Number", str, "Submit", "Back");
		}
		else {
			ShowContacts(playerid);
		}
	...
	if (dialogid == DIALOG_ENTER_NUMBER) {
        if (response)
		{
		    static
		        name[32],
			...
			strunpack(name, pData[playerid][pEditingItems]);
			
			format(str,sizeof(str),"Contact Name: %s\n\nPlease enter the phone number for this contact:",name);

		    if (isnull(inputtext) || !IsNumeric(inputtext))
		        return ShowPlayerDialog(playerid, DIALOG_ENTER_NUMBER, DIALOG_STYLE_INPUT, "Contact Number", str, "Submit", "Back");
			...
			for (new i = 0; i != MAX_CONTACTS; i ++)
			{
				if (!ContactData[playerid][i][contactExists])
				{
					...
					Servers(playerid, "You have added \"%s\" to your contacts.", name);
	                return 1;
				}
		    }
		...
		if(response)
		{
			switch (listitem) 
			{
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYVIP, DIALOG_STYLE_LIST, ""RED_E"LC:RP "WHITE_E"VIP Toys", string, "Select", "Cancel");
				}
			}
		}
	}
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 1: //colt45 9mm
				{
					...
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan 500 material dan 500 component dengan harga $7.000!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
				...
				case 2: //deagle
				{
					...
					if(pData[playerid][pMaterial] < 500) return Error(playerid, "Material tidak cukup!(Butuh: 500).");
					if(pData[playerid][pComponent] < 500) return Error(playerid, "Component tidak cukup!(Butuh: 500).");
					if(GetPlayerMoney(playerid) < 11000) return Error(playerid, "Not enough money!");
					...
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan 500 material dan 500 component dengan harga $11.000!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
				...
				case 3: //shotgun
				{
					...
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan 500 material dan 500 component dengan harga $9.000!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
				...
				case 5: //ak-47
				{
					...
					TogglePlayerControllable(playerid, 0);
					Info(playerid, "Anda membuat senjata ilegal dengan 500 material dan 500 component dengan harga $17.000!");
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
		...
		if (response) {
			switch(listitem) {
				case 0: {
					...
					pData[playerid][pTrawl] += 30;
					GivePlayerMoneyEx(playerid, -200);
					Info(playerid,"Kamu telah membeli trawl seharga $200.");
				}
				case 1: {
					if (GetPlayerMoney(playerid) < 500)
						return Error(playerid,"Not enough money!");

					pData[playerid][pTrawl] += 60;
					GivePlayerMoneyEx(playerid, -500);
					Info(playerid,"Kamu telah membeli trawl seharga $500.");
				}
			}
		}
		return 1;
	...
	if (dialogid == DIALOG_AMMUNATION) {
		if (response) {
			switch(listitem) {
				case 0: {
					if (pData[playerid][pGunLicType] != 1)
						return Error(playerid,"You Firearms License must type Handgun to buy this gun.");

					if (GetPlayerMoney(playerid) < 4000) return Error(playerid,"You need $4000 to buy this gun.");

					GivePlayerWeaponEx(playerid, 23, 300);
					GivePlayerMoneyEx(playerid, -4000);
					Info(playerid,"You've purchased Silinced Pistol for %s.", FormatMoney(4000));
				}
				case 1: {
					if (pData[playerid][pGunLicType] != 1)
						return Error(playerid,"You Firearms License must type Handgun to buy this gun.");

					if (GetPlayerMoney(playerid) < 10000) return Error(playerid,"You need $10000 to buy this gun.");

					GivePlayerWeaponEx(playerid, 24, 300);
					GivePlayerMoneyEx(playerid, -10000);
					Info(playerid,"You've purchased Desert Eagle for %s.", FormatMoney(10000));
				}
				case 2: {
					if (pData[playerid][pGunLicType] != 2)
						return Error(playerid,"You Firearms License must type Rifle Gun to buy this gun.");

					if (GetPlayerMoney(playerid) < 5000) return Error(playerid,"You need $5000 to buy this gun.");

					GivePlayerWeaponEx(playerid, 33, 300);
					GivePlayerMoneyEx(playerid, -5000);
					Info(playerid,"You've purchased Rifle for %s.", FormatMoney(5000));
				}
				case 3: {
					if (pData[playerid][pGunLicType] != 2)
						return Error(playerid,"You Firearms License must type Rifle Gun to buy this gun.");

					if (GetPlayerMoney(playerid) < 7000) return Error(playerid,"You need $7000 to buy this gun.");

					GivePlayerWeaponEx(playerid, 25, 300);
					GivePlayerMoneyEx(playerid, -7000);
					Info(playerid,"You've purchased Shotgun for %s.", FormatMoney(7000));
				}
				case 4: {
					if (pData[playerid][pGunLicType] != 3)
						return Error(playerid,"You Firearms License must type Automatic Gun to buy this gun.");

					if (GetPlayerMoney(playerid) < 15000) return Error(playerid,"You need $15000 to buy this gun.");

					GivePlayerWeaponEx(playerid, 29, 500);
					GivePlayerMoneyEx(playerid, -15000);
					Info(playerid,"You've purchased MP5 for %s.", FormatMoney(15000));
				}
				case 5: {
					if (pData[playerid][pGunLicType] != 3)
						return Error(playerid,"You Firearms License must type Automatic Gun to buy this gun.");

					if (GetPlayerMoney(playerid) < 20000) return Error(playerid,"You need $20000 to buy this gun.");
					
					GivePlayerWeaponEx(playerid, 31, 500);
					GivePlayerMoneyEx(playerid, -20000);
					Info(playerid,"You've purchased M4 for %s.", FormatMoney(20000));
				}
			}
		}
		return 1;
		...
		if (response) {
			switch (listitem) {
				case 0: {
					...
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan membeli kapal "PINK_E"%s "WHITE_E"dengan harga "LG_E"%s", GetVehicleModelName(modelid), FormatMoney(price));
					ShowPlayerDialog(playerid, DIALOG_BUYBOAT_CONFIRM, DIALOG_STYLE_MSGBOX, "Boat Dealer", tstr, "Buy", "Cancel");
				}
				case 1: {
					new modelid = 452;
					new tstr[128], price = GetVehicleCost(modelid);
		...
		if (response) {
			switch (listitem) {
				case 0: {
					...
					new tstr[128];
					pData[playerid][pBuyPvModel] = modelid;
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan menyewa kendaraan "PINK_E"%s "WHITE_E"dengan harga "LG_E"$400 / days", GetVehicleModelName(modelid));
					ShowPlayerDialog(playerid, DIALOG_RENT_BOATCONFIRM, DIALOG_STYLE_MSGBOX, "Rental Vehicles", tstr, "Rental", "Batal");
				}
				case 1: {
					new modelid = 452;
					new tstr[128];
	...
	if (dialogid == DIALOG_MOWER) {
		...
		if (response) {
			...
			SetPlayerCheckpoint(playerid, mowerpoint1, 3.0);
			InfoTD_MSG(playerid, 3000, "Follow the checkpoint!");
		} else {
			RemovePlayerFromVehicle(playerid);
			SetTimerEx("RespawnPV", 3000, false, "d", vehicleid);
		...
		if (response) {
			...
			if (pData[playerid][pSideJob] == 4) {
				for(new i; i < sizeof(TrashData); i++) {
					// SetPlayerMapIcon(playerid, 24, TrashData[i][TrashX], TrashData[i][TrashY], TrashData[i][TrashZ], 43, 0);
					TrashData[i][TrashMap] = CreateDynamicMapIcon(TrashData[i][TrashX], TrashData[i][TrashY], TrashData[i][TrashZ], 43, 0, 0, 0, playerid, 10000.0);
				}
			}
		} else {
			RemovePlayerFromVehicle(playerid);
	...
	if(dialogid == DIALOG_BUILDER2)
    {
    	new string[128];
    	if(response){
    		if(!PlayersInJob[playerid]){
			    SendClientMessage(playerid, COLOR_RED, "Anda menyelesaikan hari");
			    SendClientMessage(playerid, COLOR_RED, "Untuk kembali bekerja, kenakan seragam Anda");
				return 1;
			}
		    PlayersInJob[playerid] = 0;
			SetPlayerSkin(playerid, Oldskin2[playerid]);
		    DisablePlayerCheckpoint(playerid);
		    AddPlayerSalary(playerid, "Sidejob(Builder)", Constructor[playerid]*50);
		    format(string, sizeof(string), "{FFFAFA}Keuntungan Anda adalah {228B22}%d$ {FFFAFA}Baik? {A52A2A}%d {FFFAFA}Terima kasih telah bekerja di perusahaan Magnitudee ini)",Constructor[playerid]*50,Constructor[playerid]);
		    SendClientMessage(playerid, COLOR_LBLUE, string);
		    Constructor[playerid] = 0;
		}
		return 1;
  	}
	if(dialogid == DIALOG_BUILDER)
    {
	    if(response){
		    if(PlayersInJob[playerid]){
			    SendClientMessage(playerid, COLOR_RED, "Pergi ke luar");
			    SendClientMessage(playerid, COLOR_RED, "Anda sudah bekerja");
				return 1;
			}
		    PlayersInJob[playerid] = 1;
			Oldskin2[playerid] = GetPlayerSkin(playerid);
			SetPlayerSkin(playerid, 260);
			SetPlayerCheckpoint(playerid,-2048.0684,256.7028,35.6630,2.0);////ambil lebih banyak xd
			SendClientMessage(playerid, COLOR_LBLUE, "Gunakan campuran untuk mulai bekerja");
			SendClientMessage(playerid, COLOR_LBLUE, "Anda sudah memiliki seragam dan bekerja keluar untuk bekerja");
		}
		return 1;
  	}
    return 1;
