...
IsDetectiveCar(carid) {
	for (new i = 0; i < sizeof(DetectiveVeh); i ++) {
		if (carid == DetectiveVeh[i]) return 1;
	}
	return 0;
}
...
// MASK
new Text3D:MaskTags[MAX_PLAYERS], MaskHide[MAX_PLAYERS];

// milker
new bool:LapteVaca[MAX_PLAYERS], TimeDelayMilker[MAX_PLAYERS], PlayerHasMilker[MAX_PLAYERS];

// nelayan
new TotalTrawl[MAX_PLAYERS], PlayerHasTrawl[MAX_PLAYERS];
...
public OnGameModeInit()
{
	...
	//Dynamic CP
	ShowRoomCP = CreateDynamicCP(1294.0907,-1439.6799,13.6289, 1.0, -1, -1, -1, 5.0);
	CreateDynamic3DTextLabel("Buy Vehicle", COLOR_GREEN, 1294.0907,-1439.6799,13.6289, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, -1);
	
	ShowRoomCPRent = CreateDynamicCP(1297.1675,-1439.6792,13.6289, 1.0, -1, -1, -1, 5.0);
	CreateDynamic3DTextLabel("Rental Vehicle\n"YELLOW_E"/unrentpv", COLOR_LBLUE, 1297.1675,-1439.6792,13.6289, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, -1);
	...
	for(new x;x<sizeof(SAPDVehicles);x++)
	{
	    ...
	    SetValidVehicleHealth(SAPDVehicles[x], 2000.0);
	    SetVehicleToRespawn(SAPDVehicles[x]);
	}

	DetectiveVeh[0] = AddStaticVehicleEx(470,1545.5093,-1658.9702,5.8790,91.1937,0,1,VEHICLE_RESPAWN,1); //PATRIOT
	SetVehicleHealth(DetectiveVeh[0], 2000);
	AddVehicleComponent(DetectiveVeh[0], 1074);
	AddVehicleComponent(DetectiveVeh[0], 1010);
	DetectiveVeh[1] = AddStaticVehicleEx(426,1544.9167,-1663.0182,5.6273,92.0799,0,0,VEHICLE_RESPAWN,1); //PREMIER
	SetVehicleHealth(DetectiveVeh[1], 2000);
	AddVehicleComponent(DetectiveVeh[1], 1074);
	AddVehicleComponent(DetectiveVeh[1], 1010);
	DetectiveVeh[2] = AddStaticVehicleEx(552,1545.3135,-1668.0057,5.5818,91.5471,1,1,VEHICLE_RESPAWN,1); //UTILITY
	SetVehicleHealth(DetectiveVeh[2], 2000);
	AddVehicleComponent(DetectiveVeh[2], 1074);
	AddVehicleComponent(DetectiveVeh[2], 1010);
	DetectiveVeh[3] = AddStaticVehicleEx(427,1545.2375,-1672.0225,6.0228,89.4403,0,1,VEHICLE_RESPAWN,1); // ENFORCER
	SetVehicleHealth(DetectiveVeh[3], 2000);
	AddVehicleComponent(DetectiveVeh[3], 1074);
	AddVehicleComponent(DetectiveVeh[3], 1010);
	DetectiveVeh[4] = AddStaticVehicleEx(566,1544.3647,-1676.1589,5.6767,88.9018,0,0,VEHICLE_RESPAWN,1); //TAHOMA
	SetVehicleHealth(DetectiveVeh[4], 2000);
	AddVehicleComponent(DetectiveVeh[4], 1074);
	AddVehicleComponent(DetectiveVeh[4], 1010);
	DetectiveVeh[5] = AddStaticVehicleEx(522,1544.2665,-1680.2101,5.4592,87.2977,0,1,VEHICLE_RESPAWN,0); //NRG
	SetVehicleHealth(DetectiveVeh[5], 2000);
	AddVehicleComponent(DetectiveVeh[5], 1074);
	AddVehicleComponent(DetectiveVeh[5], 1010);
	DetectiveVeh[6] = AddStaticVehicleEx(560,1545.7566,-1684.1803,5.5955,89.7836,1,1,VEHICLE_RESPAWN,1); //SULTAN
	SetVehicleHealth(DetectiveVeh[6], 2000);
	AddVehicleComponent(DetectiveVeh[6], 1074);
	AddVehicleComponent(DetectiveVeh[6], 1010);

	for (new i; i < sizeof(DetectiveVeh); i ++) {
		SetValidVehicleHealth(DetectiveVeh[i], 2000.0);
		SetVehicleToRespawn(DetectiveVeh[i]);
	}
...
public OnGameModeExit()
{
	...
	foreach (new objid : Obj) if (Iter_Contains(Obj, objid)) {
		count2++;
		Obj_Save(objid);
	}
	printf("[Object] Number of saved: %d", count2);
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	if(!ispassenger)
	{
		...
		for (new j = 0; j < sizeof(CreatedCars); j++) if (vehicleid == CreatedCars[j])
		{
		    if (pData[playerid][pAdmin] < 1)
		    {
				...
				GetPlayerPos(playerid, slx, sly, slz);
				SetPlayerPos(playerid, slx, sly, slz);
			    Error(playerid, "You are not Admin!");
		    }
		}
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	{
	    if (IsValidDynamic3DTextLabel(jaja[playerid]))
    ...
    if (IsValidDynamic3DTextLabel(MaskTags[playerid]))
    	DestroyDynamic3DTextLabel(MaskTags[playerid]);
	...
	foreach (new i : Trawls) if (trData[i][trOwner] == pData[playerid][pID]) {
		if (IsValidDynamicObject(trData[i][trObj]))
			DestroyDynamicObject(trData[i][trObj]);

		if (IsValidDynamic3DTextLabel(trData[i][trLabel]))
...
public OnPlayerEditAttachedObject(playerid, response, index, modelid, boneid, Float:fOffsetX, Float:fOffsetY, Float:fOffsetZ, Float:fRotX, Float:fRotY, Float:fRotZ,Float:fScaleX, Float:fScaleY, Float:fScaleZ)
{
    ...
    if(weaponid)
    {
        if(response == 1)
        {
            new enum_index = weaponid - 22, weaponname[18], str[340];
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
	...
	if (checkpointid == RentBoatCP) {
		new str[128];
		format(str, sizeof(str), ""WHITE_E"Reefer $400 / Days\nSpeeder $400 / Days");
		ShowPlayerDialog(playerid, DIALOG_RENT_BOAT, DIALOG_STYLE_LIST, "Rent Boat", str, "Rent", "Close");
	}

	if (checkpointid == BuyBoatCP) {
		new str[256];
		format(str, sizeof(str), "Boat\tPrice\n");
		format(str, sizeof(str), "%s"WHITE_E"Reefer\t"GREEN_E"%s\n", str, FormatMoney(4000));
		format(str, sizeof(str), "%s"WHITE_E"Speeder\t"GREEN_E"%s\n", str, FormatMoney(8000));
		format(str, sizeof(str), "%s"WHITE_E"Tropic\t"GREEN_E"%s\n", str, FormatMoney(10000));
		format(str, sizeof(str), "%s"WHITE_E"Marquis\t"GREEN_E"%s\n", str, FormatMoney(12000));
		format(str, sizeof(str), "%s"WHITE_E"Jetmax\t"GREEN_E"%s\n", str, FormatMoney(18000));
		format(str, sizeof(str), "%s"WHITE_E"Squalo\t"GREEN_E"%s", str, FormatMoney(20000));
		ShowPlayerDialog(playerid, DIALOG_BUYBOAT, DIALOG_STYLE_TABLIST_HEADERS, "Boat Dealer", str, "Buy", "Cancel");
	}
    ...
    if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	{
		for(new i; i < sizeof(FactoryData); i++)
		{
		    if(checkpointid == FactoryData[i][FactoryCP])
		    {
		        ...
		        AddPlayerSalary(playerid,"Sidejob (Trashmaster)", cash);
		        Info(playerid, "Sidejob(Trashmaster) telah masuk ke pending salary anda!");
                ...
                for (new j; j < sizeof(TrashData); j ++) {
                	if (IsValidDynamicMapIcon(TrashData[j][TrashMap])) {
                		DestroyDynamicMapIcon(TrashData[j][TrashMap]);
                	}
                }
...
{
	if(pickupid == obrero1) {
    	ShowPlayerDialog(playerid,DIALOG_BUILDER,DIALOG_STYLE_MSGBOX,"Kamu ingin","Bekerja disini?","Iya","Tidak");
	}

	if(pickupid == obrero2) {
    	ShowPlayerDialog(playerid,DIALOG_BUILDER2,DIALOG_STYLE_MSGBOX,"Anda berakhir","Giliran? Anda akan menerima gaji Anda","Iya","Tidak");
	}

	if(pickupid == obrero3) {
    	ShowPlayerDialog(playerid,DIALOG_BUILDER3,DIALOG_STYLE_MSGBOX,"Memperhatikan","1. Pekerjaan ini terdiri dari \n2 . Ambil campuran dan \n3. Bawa dia ke lokasi konstruksi \n4. Jika Anda menjatuhkan salah satu botol campuran \nAnda harus memilih yang lain","Keberuntungan","");
	}
	return 1;
}
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if (pData[playerid][pSideJob] == 3) {
		...
		if (GetVehicleModel(vehicleid) == 572) {
			if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint1))
		    {
		        SetPlayerCheckpoint(playerid, mowerpoint2, 7.0);
				GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint2))
			{
			    SetPlayerCheckpoint(playerid, mowerpoint3, 7.0);
			    GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint3))
	        {
	            SetPlayerCheckpoint(playerid, mowerpoint4, 7.0);
	            GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint4))
	        {
	            SetPlayerCheckpoint(playerid, mowerpoint5, 7.0);
	            GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint5))
	        {
	            SetPlayerCheckpoint(playerid, mowerpoint6, 7.0);
	            GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint6))
	        {
	            SetPlayerCheckpoint(playerid, mowerpoint7, 7.0);
	            GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint7))
			{
			    SetPlayerCheckpoint(playerid, mowerpoint8, 7.0);
			    GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint8))
			{
			    SetPlayerCheckpoint(playerid, mowerpoint9, 7.0);
			    GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint9))
	        {
	            SetPlayerCheckpoint(playerid, mowerpoint10, 7.0);
	            GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint10))
	        {
	            SetPlayerCheckpoint(playerid, mowerpoint11, 7.0);
	            GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
			if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint11))
			{
			    SetPlayerCheckpoint(playerid, mowerpoint12, 7.0);
			    GameTextForPlayer(playerid, "~g~Bersih!", 3000, 3);
			}
	        if (IsPlayerInRangeOfPoint(playerid, 7.0,mowerpoint12))
	        {
		 		pData[playerid][pSideJob] = 0;
				pData[playerid][pSideJobTime] += 900;
				DisablePlayerCheckpoint(playerid);
	...
	new str[256];
   	if(PlayersToPoint(2.0, playerid,-2048.0684,256.7028,35.6630))///Ambil saja campuranya
   	{
	  Mescla[playerid] = 1;
	  ApplyAnimation(playerid,"CARRY","crry_prtial",4.1,0,1,1,1,1);//ditinjau itu baik-baik saja
	  SetPlayerAttachedObject(playerid, 2, 1329, 1,0.20,0.36,0.0,0.0,90.0, 0.0, 0.4, 0.3, 0.6);
      SetPlayerCheckpoint(playerid,-2019.7251,303.4937,54.9075,2.0);///Di sini Anda mengirimkan campurannya
    }         ////-2020.4305, 303.4898, 54.2458

    if(PlayersToPoint(2.0, playerid,-2019.7251,303.4937,54.9075))///pengiriman campuran
	{
	  Mescla[playerid] = 0;
	  ///RemovePlayerAttachedObject(playerid,1);
	  //ApplyAnimation(playerid,"CARRY","putdwn",4.1,0,1,1,1,1);/// sesuatu dalam animasi gagal
	  ApplyAnimation(playerid,"CARRY","putdwn",1,0,0,0,0,0);
	  Constructor[playerid]++;
	  if(IsPlayerAttachedObjectSlotUsed(playerid,2)) RemovePlayerAttachedObject(playerid,2);
	  format(str, sizeof(str), "Anda memberikan campuran itu untuk lebih {A52A2A}%d",Constructor[playerid]);
	  SendClientMessage(playerid, COLOR_SYSTEM, str);
      SetPlayerCheckpoint(playerid,-2048.0684,256.7028,35.6630,2.0);///ambil sekali lagi
    }
	//DisablePlayerCheckpoint(playerid);
	return 1;
}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    {
        if (PlayerHasMilker[playerid] == 0) {
	        if (TimeDelayMilker[playerid] >= gettime()) {
	            Error(playerid, "ERROR: {00FF00}Kamu harus menunggu %d detik untuk memerah susu sapi lagi.", (TimeDelayMilker[playerid] - gettime()));
	            return 1;
	        }
	        if(LapteVaca[playerid]) return SendClientMessage(playerid,-1, "[PETERNAK] : {00FF00}Anda sudah mengambil susu sapi! Pergi ke gudang! Dan jangan malas!");
	        for(new i = 0x0; i != sizeof(PozitieVaca); i++)
	        {
	            if(IsPlayerInRangeOfPoint(playerid,2.0,PozitieVaca[i][0], PozitieVaca[i][1], PozitieVaca[i][2]))
	            {
	                ...
	                switch(random(5))
					{
					case 0: LapteVaca[playerid] = !LapteVaca[playerid], PosMilker[i]--, Info(playerid,"[PETERNAK]:{999999} Bravo, Anda mengambil susu dari sapi");
					case 1: Info(playerid," {FF00FF}BERHENTI! Sapi itu menderita mastitis, dia tidak memerah susu!{00FF00}(SAKIT)");
					case 2: LapteVaca[playerid] = !LapteVaca[playerid], PosMilker[i]--, Info(playerid,"[PETERNAK]:{999999} Bravo, Anda mengambil susu dari sapi.");
					case 3: Info(playerid," {FF00FF}BERHENTI! Sapi itu menderita mastitis, dia tidak memerah susu!{00FF00}(SAKIT)");
					}
	                if(PosMilker[i] == 0) return SendClientMessage(playerid,-1, "SERVER : {FFFFFF}Sapi ini diperah oleh pemain lain.");
	                UpdateDynamic3DTextLabelText(labelMilker[i], 0xFFFFFFFF, "{FFFFFF} Ambil susu sapi perah\ntekan {FFFFFF}H");
	                TimeDelayMilker[playerid] = gettime() + 600;
	                PlayerHasMilker[playerid] = 1;
	                break;
	            }
			}
		}
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	else if(newstate == PLAYER_STATE_DRIVER)
    {
		...
		foreach(new pv : PVehicles)
		{
			if(vehicleid == pvData[pv][cVeh])
			{
				if(IsABike(vehicleid) || GetVehicleModel(vehicleid) == 424 || IsABoat(vehicleid))
				{
					if(pvData[pv][cLocked] == 1)
					{
						RemovePlayerFromVehicle(playerid);
						...
						//GetPlayerPos(playerid, slx, sly, slz);
						//SetPlayerPos(playerid, slx, sly, slz);
						Error(playerid, "This vehicle is locked by owner.");
						return 1;
					}
				}
			}
		...
		if (IsAMowerVeh(vehicleid)) {
			ShowPlayerDialog(playerid, DIALOG_MOWER, DIALOG_STYLE_MSGBOX, "Side Job - Mower", "Anda akan bekerja sebagai pemotong rumput dengan Mower?", "Start Job", "Cancel");
		}
	    ...
	    if(IsATrashCar(veh))
	    {
		    if(LoadedTrash[veh] > 0 && pData[playerid][pSideJob] == 4) {
		        new string[128];
		        format(string, sizeof(string), "TRASHMASTER JOB: {FFFFFF}This vehicle has {F39C12}%d {FFFFFF}trash bags which is worth {2ECC71}$%d.", LoadedTrash[veh], LoadedTrash[veh] * TRASH_BAG_VALUE);
				SendClientMessage(playerid, 0x2ECC71FF, string);
		        ...
		        ShowPlayerDialog(playerid, DIALOG_TRASHMASTER, DIALOG_STYLE_MSGBOX, "Sidejob Trashmaster", "Anda akan bekerja sebagai pengangkut sampah?", "Start Job", "Cancel");
		    }

			Trash_ShowCapacity(playerid);
			SetPVarInt(playerid, "LastVehID", veh);
		}
...
public OnPlayerUpdate(playerid)
{
	...
	{
		if (!MaskHide[playerid])
	    {
            foreach (new pid : Player) {
				ShowPlayerNameTagForPlayer(pid, playerid, 0);
			}
		    format(str, sizeof(str), "Mask_#%d", pData[playerid][pMaskID]);

	        MaskHide[playerid] = 1;
	        MaskTags[playerid] = CreateDynamic3DTextLabel(str, COLOR_WHITE, 0.0, 0.0, 0.2, 8.0, playerid, INVALID_VEHICLE_ID, 0, -1, -1);
	    }
	}
	if (!pData[playerid][pMaskOn] && MaskHide[playerid])
	{
	    foreach (new j : Player) {
			ShowPlayerNameTagForPlayer(j, playerid, 1);
		}
		if (IsValidDynamic3DTextLabel(MaskTags[playerid]))
		    DestroyDynamic3DTextLabel(MaskTags[playerid]);

	    MaskTags[playerid] = Text3D:INVALID_3DTEXT_ID;
	}

	new animlib[32];
   	new animname[32];

 	if(Mescla[playerid] == 1 && !PlayersToPoint(2.0,playerid,-2048.0684,256.7028,35.6630)){///aca recojes la mescla
		...
		if(strcmp(animname, "IDLE_STANCE", true) != 0 && strcmp(animname, "RUN_CIVI", true) != 0 && strcmp(animname, "WALK_CIVI", true) != 0 && strcmp(animname, "crry_prtial", true) != 0 && strcmp(animname, "SPRINT_PANIC", true) != 0 && strcmp(animname, "RUN_PLAYER", true) != 0 && strcmp(animname, "WALK_PLAYER", true) != 0 && strcmp(animname, "SPRINT_CIVI", true) != 0){
			Mescla[playerid] = 0;
			SendClientMessage(playerid, COLOR_RED, "Anda telah membuang campurannya, kembali ke tempat Anda mengambilnya, Anda memiliki diskon 10%!");
			if(IsPlayerAttachedObjectSlotUsed(playerid,2)) RemovePlayerAttachedObject(playerid,2);///ini bagus
			ApplyAnimation(playerid,"CARRY","crry_prtial",4.1,0,1,1,0,1);
			SetPlayerCheckpoint(playerid,-2048.0684,256.7028,35.6630,2.0);///di sini campuran pengambilan checkpoint
 		}
 	}
	return 1;
}
		...
		if(vehicleid == pvData[ii][cVeh] && pvData[ii][cRent] == 0)
		{
			...
			if(pvData[ii][cInsu] > 0)
    		{
				...
				foreach(new pid : Player) if (pvData[ii][cOwner] == pData[pid][pID])
        		{
            		...
            		Info(pid, "Kendaraan Anda dihancurkan oleh %s.", pData[killerid][pName]);
				}
				if(IsValidVehicle(pvData[ii][cVeh]))
					DestroyVehicle(pvData[ii][cVeh]);
...
ptask PlayerVehicleUpdate[200](playerid)
{
	...
	if(IsValidVehicle(vehicleid))
	{
		...
		if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			if(IsValidVehicle(vehicleid) && fHealth <= 0.0)
			{
				SetValidVehicleHealth(vehicleid, 0.0);
				SwitchVehicleEngine(vehicleid, false);
			}
		}
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
...
ptask PlayerUpdate[999](playerid)
{
    ...
    if (GetPlayerSpeed(playerid) > 210 && pData[playerid][pAdmin] < 1)
	{
	    if (!IsAPlane(GetPlayerVehicleID(playerid)) && GetPlayerState(playerid) != PLAYER_STATE_PASSENGER) {
	        SendStaffMessage(COLOR_YELLOW, "%s (%d) has possibly used speed hacks (%.0f mph).", ReturnName(playerid), playerid, GetPlayerSpeed(playerid));
		}
	}
	if (GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_USEJETPACK && pData[playerid][pJetpack] == 0)
	{
    	SendStaffMessage(COLOR_YELLOW, "%s (%d) has spawned a jetpack using hacks.", ReturnName(playerid), playerid);
	}
	//Weapon Atth
