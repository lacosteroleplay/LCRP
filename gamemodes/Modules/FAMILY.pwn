...
{
	new dquery[3048];
	format(dquery, sizeof(dquery), "UPDATE familys SET name='%s', leader='%s', motd='%s', color='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', intposx='%f', intposy='%f', intposz='%f', intposa='%f', fint='%d', rankname1='%s', rankname2='%s', rankname3='%s', rankname4='%s', rnakname5='%s', rankname6='%s'",
	fData[id][fName],
	fData[id][fLeader],
	fData[id][fMotd],
	fData[id][fColor],
...
function LoadFamilys()
{
    new rows = cache_num_rows();
 	if(rows)
  	{
   		new fid, name[100], leader[MAX_PLAYER_NAME], motd[100];
   		new rankname1[32],rankname2[32],rankname3[32],rankname4[32],rankname5[32],rankname6[32];
		for(new i; i < rows; i++)
		{
  			cache_get_value_name_int(i, "ID", fid);
	    	cache_get_value_name(i, "name", name);
			format(fData[fid][fName], 100, name);
		    ...
		    cache_get_value_name(i, "rankname1", rankname1);
		    format(fData[fid][fRankName1], 32, rankname1);
		    cache_get_value_name(i, "rankname2", rankname2);
		    format(fData[fid][fRankName2], 32, rankname2);
		    cache_get_value_name(i, "rankname3", rankname3);
		    format(fData[fid][fRankName3], 32, rankname3);
		    cache_get_value_name(i, "rankname4", rankname4);
		    format(fData[fid][fRankName4], 32, rankname4);
		    cache_get_value_name(i, "rankname5", rankname5);
		    format(fData[fid][fRankName5], 32, rankname5);
		    cache_get_value_name(i, "rankname6", rankname6);
		    format(fData[fid][fRankName6], 32, rankname6);
...
{
	new rank, otherid, rankname[32];
	if(pData[playerid][pFamilyRank] < 6)
		return Error(playerid, "You must family leader!");
		
	if(sscanf(params, "uds[32]", otherid, rank, rankname))
        return Usage(playerid, "/fsetrankname [playerid/PartOfName] [rank 1-6] [rank name]");
	...
	if (strlen(rankname) > 32)
		return Error(playerid,"Max character is 32 characters.");

	new id = pData[playerid][pFamily];

	switch (rank) {
		case 1: {
			strmid(fData[id][fRankName1], rankname, 0, strlen(rankname), 32);
			Family_Save(id);
			Info(playerid,"You has set rank 1 name to %s.",rankname);
		}
		case 2: {
			strmid(fData[id][fRankName2], rankname, 0, strlen(rankname), 32);
			Family_Save(id);
			Info(playerid,"You has set rank 2 name to %s.",rankname);
		}
		case 3: {
			strmid(fData[id][fRankName3], rankname, 0, strlen(rankname), 32);
			Family_Save(id);
			Info(playerid,"You has set rank 3 name to %s.",rankname);
		}
		case 4: {
			strmid(fData[id][fRankName4], rankname, 0, strlen(rankname), 32);
			Family_Save(id);
			Info(playerid,"You has set rank 4 name to %s.",rankname);
		}
		case 5: {
			strmid(fData[id][fRankName5], rankname, 0, strlen(rankname), 32);
			Family_Save(id);
			Info(playerid,"You has set rank 5 name to %s.",rankname);
		}
		case 6: {
			strmid(fData[id][fRankName6], rankname, 0, strlen(rankname), 32);
			Family_Save(id);
			Info(playerid,"You has set rank 6 name to %s.",rankname);
		}
	}
	return 1;
}
...
CMD:fquit(playerid, params[])
{
	...
	Info(playerid,"You has been leave from your family.");
	return 1;
}
...
CMD:families(playerid) {
	new str[1024];
	format(str, sizeof(str), "ID\tName\tLeader\n");
	for (new i; i < MAX_FAMILY; i ++) if (strcmp(fData[i][fName], "None", true)) {
		format(str, sizeof(str), "%s%d\t%s\t%s\n",str,i,fData[i][fName],fData[i][fLeader]);
		format(str, sizeof(str), "%s\n", str);
	}
	format(str, sizeof(str), "%s", str);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, "All Family", str, "Close", "");
	return 1;
}
 	...
 	if(rows)
  	{
		for(new i = 0; i != rows; i++)
		{
			...
			if(IsPlayerConnected(pid))
				format(query, sizeof(query), ""GREEN_E"(ONLINE) ");
			else
				format(query, sizeof(query), ""RED_E"(OFFLINE) ");
			
			cache_get_value_index_int(i, 1, frank);
			if(frank == 1)
			{
				format(query, sizeof(query), ""FAMILY_E"%s(1)",fData[fid][fRankName1]);
			}
			else if(frank == 2)
			{
				format(query, sizeof(query), ""FAMILY_E"%s(2)",fData[fid][fRankName2]);
			}
			else if(frank == 3)
			{
				format(query, sizeof(query), ""FAMILY_E"%s(3)",fData[fid][fRankName3]);
			}
			else if(frank == 4)
			{
				format(query, sizeof(query), ""FAMILY_E"%s(4)",fData[fid][fRankName4]);
			}
			else if(frank == 5)
			{
				format(query, sizeof(query), ""FAMILY_E"%s(5)",fData[fid][fRankName5]);
			}
			else if(frank == 6)
			{
				format(query, sizeof(query), ""FAMILY_E"%s(6)",fData[fid][fRankName6]);
			}
			else
			{
				format(query, sizeof(query), ""FAMILY_E"None(0)");
			}
