...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
	    cache_get_value_name_int(0, "aktif", pData[playerid][pActive]);
		if (pData[playerid][pActive] == 0) {
			ShowPlayerDialog(playerid,DIALOG_UNUSED,DIALOG_STYLE_MSGBOX,"Account is not activated",""WHITE_E"This account is not activated, please check your email to activate your account","Ok","");
			KickEx(playerid);
		} else {
			...
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "LC:RP - Login", string, "Login", "Abort");
		}
		//pData[playerid][LoginTimer] = SetTimerEx("OnLoginTimeout", SECONDS_TO_LOGIN * 1000, false, "i", playerid);
	}
	else
	{
		ShowPlayerDialog(playerid,DIALOG_UNUSED,DIALOG_STYLE_MSGBOX,"Account not registered",""WHITE_E"This username is not registered, please register at "YELLOW_E"https://ucp.lacosteroleplay.com","Ok :(","");
		KickEx(playerid);
...
function CheckBan(playerid)
{
	if(cache_num_rows() > 0)
	{
		...
		else
		{
			...
			{
				new lstr[512];
				format(lstr, sizeof(lstr), "{FF0000}You are banned from this server!\n\n"LB2_E"Ban Info:\n{FF0000}Name: {778899}%s\n{FF0000}IP: {778899}%s\n{FF0000}Admin: {778899}%s\n{FF0000}Ban Date: {778899}%s\n{FF0000}Ban Reason: {778899}%s\n{FF0000}Ban Time: {778899}Permanent\n\n{FFFFFF}Feel that you were wrongfully banned? Appeal at https://forum.lacosteroleplay.com", BannedName, PlayerIP, PlayerName, ReturnDate(banDate), Reason);
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"BANNED", lstr, "Exit", "");
			}
			else
			{
				new lstr[512];
				format(lstr, sizeof(lstr), "{FF0000}You are banned from this server!\n\n"LB2_E"Ban Info:\n{FF0000}Name: {778899}%s\n{FF0000}IP: {778899}%s\n{FF0000}Admin: {778899}%s\n{FF0000}Ban Date: {778899}%s\n{FF0000}Ban Reason: {778899}%s\n{FF0000}Ban Time: {778899}%s\n\n{FFFFFF}Feel that you were wrongfully banned? Appeal at https://forum.lacosteroleplay.com", BannedName, PlayerIP, PlayerName, ReturnDate(banDate), Reason, ReturnTimelapse(gettime(), banTime_Int));
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"BANNED", lstr, "Exit", "");
			}
			KickEx(playerid);
			return 1;
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "dice", pData[playerid][pDice]);
	cache_get_value_name_int(0, "tollcard", pData[playerid][pTollCard]);
	...
	cache_get_value_name_int(0, "workbiz", pData[playerid][pWorkerBiz]);
	...
	cache_get_value_name_int(0, "factiondiv", pData[playerid][pFactionDiv]);
	...
	cache_get_value_name_int(0, "trawl", pData[playerid][pTrawl]);
	...
	cache_get_value_name_int(0, "trucklic", pData[playerid][pTruckerLic]);
	cache_get_value_name_int(0, "trucklic_time", pData[playerid][pTruckerLicTime]);
	...
	cache_get_value_name_int(0, "gunlic_type", pData[playerid][pGunLicType]);
	...
	mysql_tquery(g_SQL,string,"LoadContacts","d",playerid);

	mysql_format(g_SQL,string,sizeof(string),"SELECT * FROM `vouchers` WHERE username = '%e' AND status = '1', AND claim = '0'",pData[playerid][pName]);
	mysql_tquery(g_SQL,string,"Load_Vouchers","s",pData[playerid][pName]);
...
function SafeLogin(playerid)
{
	...
	if(!IsValidRoleplayName(pData[playerid][pName]))
    {
        ...
        Error(playerid, "Sebagai contoh, Jesse_Kendrich, Nick_Raymond, dll.");
        KickEx(playerid);
    }
}
...
{	
	...
	foreach(new ii : Player)
	{
		...
		if(pData[ii][pFlyLic] > 0)
		{
			if(pData[ii][pFlyLicTime] != 0 && pData[ii][pFlyLicTime] <= gettime())
			{
				Info(ii, "Masa berlaku Flylic License anda telah habis, silahkan perpanjang kembali!");
				pData[ii][pFlyLic] = 0;
				pData[ii][pFlyLicTime] = 0;
			}
		}
		if(pData[ii][pTruckerLic] > 0)
		{
			if(pData[ii][pTruckerLicTime] != 0 && pData[ii][pTruckerLicTime] <= gettime())
			{
				Info(ii, "Masa berlaku Trucker License anda telah habis, silahkan perpanjang kembali!");
				pData[ii][pTruckerLic] = 0;
				pData[ii][pTruckerLicTime] = 0;
			}
		}
		if(pData[ii][pLumberLic] > 0)
		{
			...
			{
				Info(ii, "Masa berlaku Lumberjack License anda telah habis, silahkan perpanjang kembali!");
				pData[ii][pLumberLic] = 0;
				pData[ii][pLumberLicTime] = 0;
			}
		}
		if(pData[ii][pGunLic] > 0)
		{
			if(pData[ii][pGunLicTime] != 0 && pData[ii][pGunLicTime] <= gettime())
			{
				Info(ii, "Masa berlaku Firearms License anda telah habis, silahkan perpanjang kembali!");
				pData[ii][pGunLic] = 0;
				pData[ii][pGunLicTime] = 0;
				pData[ii][pGunLicType] = 0;
			}
		}
