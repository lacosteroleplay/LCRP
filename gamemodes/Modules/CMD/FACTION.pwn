...
CMD:factionhelp(playerid)
{
	...
	{
		new str[3500];
		strcat(str, ""BLUE_E"SAPD: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank /setdivision\n");
		strcat(str, ""WHITE_E"SAPD: /sapdonline /(un)cuff /tazer /detain /arrest /release /flare /destroyflare /checkveh\n");
		strcat(str, ""BLUE_E"SAPD: /take /spike /destroyspike /destroyallspike /trace /roadblock /createlic\n");
	...
	{
		new str[3500];
		strcat(str, ""LB_E"SAGS: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank /setdivision\n");
	...
	{
		new str[3500];
		strcat(str, ""PINK_E"SAMD: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank /setdivision\n");
		strcat(str, ""WHITE_E"SAMD: /samdonline /loadinjured /dropinjured /ems /findems /heal /salve\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
	...
	{
		new str[3500];
		strcat(str, ""ORANGE_E"SANA: /locker /or (/r)adio /od /d(epartement) (/gov)ernment (/m)egaphone /invite /uninvite /setrank /setdivision\n");
	...
	{
		new str[3500];
		strcat(str, ""WHITE_E"Family: /fsave /f(amily) /finvite /funinvite /fsetrank /fsetrankname\n");
...
{
	new rank, otherid;
	if(pData[playerid][pFactionRank] < 9)
		return Error(playerid, "You must be High Command Team!");
		
	if(sscanf(params, "ud", otherid, rank))
        return Usage(playerid, "/setrank [playerid/PartOfName] [rank 1-10]");
	...
	if(rank < 1 || rank > 10)
		return Error(playerid, "rank must 1 - 10 only");
...
{
	new division, otherid;
	if(pData[playerid][pFactionRank] < 9)
		return Error(playerid, "You must be High Command Team!");
		
	if(sscanf(params, "ud", otherid, division)) {
		Usage(playerid, "/setdivision [playerid/PartOfName] [division]");
		switch (pData[playerid][pFaction]) {
			case 1: Info(playerid,"[DIVISIONS]: 1 = General, 2 = TRU, 3 = SGL, 4 = SWAT, 5 = CI");
			case 2: Info(playerid,"[DIVISIONS]: 1 = Security, 2 = Staff, 3 = Lawyer, 4 = PR, 5 = Assistant, 6 = Governor");
			case 3: Info(playerid,"[DIVISIONS]: 1 = Field, 2 = Hospital, 3 = Fire");
			case 4: Info(playerid,"[DIVISIONS]: 1 = Staff, 2 = Reporter, 3 = Cameraman, 4 = Studio");
		}
		return true;
	}	
	...
	if(pData[otherid][pFaction] != pData[playerid][pFaction])
		return Error(playerid, "This player is not in your faction!");

	pData[otherid][pFactionDiv] = division;
	Info(playerid,"You has been set %s division to %s.",ReturnName(otherid),GetFactionDivision(playerid));
	Info(otherid,"%s has set your division to %s.",ReturnName(playerid),GetFactionDivision(playerid));
	return 1;
}
...
CMD:take(playerid,params[])
{
	...
	new otherid, items[24];
	if (sscanf(params, "us[24]", otherid, items)) {
		Usage(playerid,"/take [playerid/name] [items]");
		Info(playerid,"[ITEMS]: weapon, marijuana, drivelic");
		return 1;
	}
	if(!IsPlayerConnected(otherid) || otherid == INVALID_PLAYER_ID)
        return Error(playerid, "Player not connected!");
	...
	if (!strcmp(items, "weapon", true)) {
		ResetPlayerWeaponsEx(otherid);
		Info(playerid,"You have been take %s all weapons.",ReturnName(otherid));
		Info(otherid,"Officer %s has taked your all weapons.",ReturnName(playerid));
	} else if (!strcmp(items, "marijuana", true)) {
		pData[otherid][pMarijuana] = 0;
		Info(playerid,"You has been take %s all marijuana's.",ReturnName(otherid));
		Info(otherid,"Officer %s has taked your all marijuana's.",ReturnName(playerid));
	} else if (!strcmp(items, "drivelic", true)) {
		pData[otherid][pDriveLic] = 0;
		pData[otherid][pDriveLicTime] = 0;
		Info(playerid,"You has been take %s driving licenses.",ReturnName(otherid));
		Info(otherid,"Officer %s has taked your driving licenses.",ReturnName(playerid));
	}
...
CMD:trace(playerid, params[])
{
	...
	new Float:x, Float:y, Float:z, otherid;

	if (sscanf(params,"u",otherid))
		return Usage(playerid,"/trace [playerid/name]");
		
	if (pData[otherid][pPhoneOff] == 1)
	    return Error(playerid,"This player phone is off.");

	GetPlayerPos(otherid,x,y,z);
	Servers(playerid,"[TRACE] %s last position is %s!",pData[otherid][pName],GetLocation(x,y,z));
	return 1;
}
...
CMD:createlic(playerid,params[])
{
    ...
    if (pData[playerid][pFactionDiv] != 3)
    	return Error(playerid,"You are not Staff and Gun Licenses");

    new otherid, option[64], value[24];
    if (sscanf(params, "us[64]S()[24]", otherid, option, value)) {
    	Usage(playerid,"/createlic [playerid/name] [licenses]");
    	Info(playerid,"[LICENSES]: trucker, lumberjack, flying, firearms");
    	return 1;
    }
    if (!strcmp(option, "trucker", true)) {
    	if (pData[otherid][pTruckerLic] == 1) return Error(playerid,"This player have active Trucker License");

    	pData[otherid][pTruckerLic] = 1;
    	pData[otherid][pTruckerLicTime] = gettime() + (16 * 86400);
    	Info(playerid,"You has created Trucker License for %s.", ReturnName(otherid));
    	Info(otherid,"Officer %s has give Trucker License to you. Valid for 15 Days.",ReturnName(playerid));
    } else if (!strcmp(option, "lumberjack", true)) {
    	if (pData[otherid][pLumberLic] == 1) return Error(playerid,"This player have active Lumberjack License");

    	pData[otherid][pLumberLic] = 1;
    	pData[otherid][pLumberLicTime] = gettime() + (16 * 86400);
    	Info(playerid,"You has created Lumberjack License for %s.", ReturnName(otherid));
    	Info(otherid,"Officer %s has give Lumberjack License to you. Valid for 15 Days.",ReturnName(playerid));
    } else if (!strcmp(option, "flying", true)) {
    	if (pData[otherid][pFlyLic] == 1) return Error(playerid,"This player have active Flying License");

    	pData[otherid][pFlyLic] = 1;
    	pData[otherid][pFlyLicTime] = gettime() + (16 * 86400);
    	Info(playerid,"You has created Flying License for %s.", ReturnName(otherid));
    	Info(otherid,"Officer %s has give Flying License to you. Valid for 15 Days.",ReturnName(playerid));
    } else if (!strcmp(option, "firearms", true)) {
    	new type;
    	if (sscanf(value, "d", type)) {
    		Usage(playerid,"/createlic [playerid/name] firearms [type]");
    		Info(playerid,"[TYPES]: 1 = Handgun, 2 = Rifle Gun, 3 = Automatic Gun.");
    		return 1;
    	}

    	if (pData[otherid][pGunLic] == 1) return Error(playerid,"This player have active Firearms License");

    	pData[otherid][pGunLic] = 1;
    	pData[otherid][pGunLicTime] = gettime() + (16 * 86400);
    	pData[otherid][pGunLicType] = type;

    	new tipe[24];
    	if (pData[otherid][pGunLicType] == 1) tipe = "Handgun";
    	else if (pData[otherid][pGunLicType] == 2) tipe = "Rifle Gun";
    	else if (pData[otherid][pGunLicType] == 3) tipe = "Automatic Gun";

    	Info(playerid,"You has created Firearms License for %s.", ReturnName(otherid));
    	Info(otherid,"Officer %s has give Firearms License to you. Valid for 15 Days. Type: %s.",ReturnName(playerid),tipe);
    }
...
CMD:heal(playerid, params[])
{
    ...
    if(!pData[otherid][pInjured]) {
		...
		Servers(playerid,"HEAL: You have been healed %s.",ReturnName(otherid));
		Info(otherid,"HEAL: %s has healed your character.",ReturnName(playerid));
	} else {
		...
		SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s has healed %s with medkit tools.", ReturnName(playerid), ReturnName(otherid));
		Info(otherid, "HEAL: Officer %s has healed your character.", ReturnName(playerid));
	}
	return 1;
}
...
CMD:apotek(playerid,params[])
{
	if(pData[playerid][pFaction] != 3)
        return Error(playerid, "You must be a samd officer.");

	if (!IsPlayerInRangeOfPoint(playerid,2.5,1449.27,-32.31,1000.92))
		return Error(playerid,"You must in ASGH!");
