CMD:acmds(playerid)
{
	...
	/reports /ar /dr /vmodels /vehname /apv /aveh /gotoveh /getveh /respawnveh /respawnrad /ans\n\
	/up /dn /fd /ft /gotols /gotosf /gotolv /gentercar /entercar /setvw /setint");
	...
	/veh /destroyveh /destroycar /destroycars");
	...
	strcat(line3, "\n"BLUE_E"LC:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"LC:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:hcmds(playerid)
{
	...
	/kick /slap /caps /acuff /auncuff /reports /ar /dr /ans");
...
CMD:a(playerid, params[])
{
	...
	new mstr[512];
	format(mstr, sizeof(mstr), ""RED_E"[Admin Chat] %s "LG_E"%s(%i): "WHITE_E"%s", GetStaffRank(playerid), pData[playerid][pAdminname], playerid, params);
	foreach(new ii : Player) 
	{
...
CMD:respawnsapd(playerid, params[])
{
	...
	for (new i; i < sizeof(DetectiveVeh); i ++) {
		if (IsVehicleEmpty(DetectiveVeh[i])) {
			SetVehicleToRespawn(DetectiveVeh[i]);
			SetValidVehicleHealth(DetectiveVeh[i], 2000.0);
			SetVehicleFuel(DetectiveVeh[i], 1000);
			SwitchVehicleDoors(DetectiveVeh[i], false);
		}
	}
...
CMD:respawnjobs(playerid, params[])
{
	...
	for(new x;x<sizeof(SweepVeh);x++)
	{
		if(IsVehicleEmpty(SweepVeh[x]))
		{
			...
			SetValidVehicleHealth(SweepVeh[x], 1000);
	...
	for(new xx;xx<sizeof(BusVeh);xx++)
	{
		if(IsVehicleEmpty(BusVeh[xx]))
		{
			...
			SetValidVehicleHealth(BusVeh[xx], 1000);
			SetVehicleFuel(BusVeh[xx], 100);
			SwitchVehicleDoors(BusVeh[xx], false);
		}
	}
	for (new i; i < sizeof(MowerVeh); i ++) {
		if (IsVehicleEmpty(MowerVeh[i])) {
			SetVehicleToRespawn(MowerVeh[i]);
			SetValidVehicleHealth(MowerVeh[i], 1000);
			SetVehicleFuel(MowerVeh[i], 100);
			SwitchVehicleDoors(MowerVeh[i], false);
		}
	}
	for (new d; d < sizeof(TrashVeh); d ++) {
		if (IsVehicleEmpty(TrashVeh[d])) {
			SetVehicleToRespawn(TrashVeh[d]);
			SetValidVehicleHealth(TrashVeh[d], 1000);
			SetVehicleFuel(TrashVeh[d], 100);
			SwitchVehicleDoors(TrashVeh[d], false);
		}
	}
...
CMD:gotols(playerid)
{
    ...
    SetPlayerPosition(playerid,1482.0356,-1724.5726,13.5469,750);
    Info(playerid, "You have been teleported to LS.");
	return 1;
}
...
CMD:gotosf(playerid)
{
    ...
    SetPlayerPosition(playerid,-1425.8307,-292.4445,14.1484,750);
    Info(playerid, "You have been teleported to SF.");
	return 1;
}
...
CMD:gotolv(playerid)
{
    ...
    SetPlayerPosition(playerid,1686.0118,1448.9471,10.7695,750);
    Info(playerid, "You have been teleported to LV.");
	return 1;
}
...
// {
// 	if (pData[playerid][pAdmin] < 1)
// 	    return PermiasionError(playerid);
...
CMD:setfaction(playerid, params[])
{
    ...
    if(sscanf(params, "udd", otherid, fid, rank))
        return Usage(playerid, "/setfaction [playerid/PartOfName] [1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW] [rank 1-10]");
...
CMD:destroycars(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_RED, "CREATEDVEH: %s has destroy all created vehicle", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:destroycar(playerid, params[])
{
	...
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		if(newid != INVALID_VEHICLE_ID)
		{
		    ...
		    Servers(playerid, "CREATEDVEH: Car id %d has been destroyed!", carid);
		}
	}
	return 1;
}
...
CMD:veh(playerid, params[]) {
	...
	for (new i = 0; i < sizeof(CreatedCars); i++) {
	    if (CreatedCars[i] == INVALID_VEHICLE_ID) {
		    ...
		    CreatedCars[i] = CreateVehicle(model[0], x, y, z, a, color1, color2, 0);

		    if(GetPlayerInterior(playerid) != 0)
		        LinkVehicleToInterior(CreatedCars[i], GetPlayerInterior(playerid));

		    if(GetPlayerVirtualWorld(playerid) != 0)
		        SetVehicleVirtualWorld(CreatedCars[i], GetPlayerVirtualWorld(playerid));

		    if(IsABoat(CreatedCars[i]) || IsAPlane(CreatedCars[i]) || IsAHelicopter(CreatedCars[i]))
		        PutPlayerInVehicle(playerid, CreatedCars[i], 0);

		    SetVehicleNumberPlate(CreatedCars[i], "STATIC");
...
{
	if(pData[playerid][pAdmin] < 6 && !IsPlayerAdmin(playerid))
		return PermissionError(playerid);
...
CMD:spy(playerid,params[])
{
	...
	if (pData[playerid][pSPY] == 0) {
	    pData[playerid][pSPY] = 1;
	    SendClientMessage(playerid, COLOR_RED, "SPY: {FFFFFF}You has enabled spy!");
	} else {
	    pData[playerid][pSPY] = 0;
	    SendClientMessage(playerid, COLOR_RED, "SPY: {FFFFFF}You has disabled spy!");
	}
	return 1;
}
...
CMD:gmx(playerid)
{
	...
	if (ServerRestart == 1)
	    return Error(playerid,"Saat ini server sedang melakukan restart");

    SendClientMessageToAllEx(COLOR_YELLOW, "GMX: "WHITE_E"Administrator %s telah merestart server dalam waktu 30 detik.");
    SendClientMessageToAllEx(COLOR_YELLOW, "GMX: "WHITE_E"Semua player harap /q agar tidak terjadi roleback.");
    ServerRestart = 1;
