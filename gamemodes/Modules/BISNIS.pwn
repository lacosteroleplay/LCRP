...
enum bisinfo
{
	...
	bMap,
	Text3D:bLabel,
	bWorker1[MAX_PLAYER_NAME],
	bWorker2[MAX_PLAYER_NAME],
	bWorker3[MAX_PLAYER_NAME],
	bWorker4[MAX_PLAYER_NAME],
	bWorker5[MAX_PLAYER_NAME],
	bWork
};
...
Bisnis_Save(id)
{
	...
	format(cQuery, sizeof(cQuery), "%sworker1='%s', worker2='%s', worker3='%s', worker4='%s', worker5='%s', work='%d' WHERE ID='%d'",
	cQuery,
	bData[id][bWorker1],
	bData[id][bWorker2],
	bData[id][bWorker3],
...
Player_BisnisWorker(playerid, id)
{
	if (!IsPlayerConnected(playerid)) return 0;
	if (id == -1) return 0;
	if(!strcmp(bData[id][bWorker1], pData[playerid][pName], true)) return 1;
	if(!strcmp(bData[id][bWorker2], pData[playerid][pName], true)) return 1;
	if(!strcmp(bData[id][bWorker3], pData[playerid][pName], true)) return 1;
	if(!strcmp(bData[id][bWorker4], pData[playerid][pName], true)) return 1;
	if(!strcmp(bData[id][bWorker5], pData[playerid][pName], true)) return 1;
	return 0;
}

Setbiz_Worker(bizid, id, playerid)
{
	switch (id) {
		case 1: strmid(bData[bizid][bWorker1], pData[playerid][pName], 0, strlen(pData[playerid][pName]), 256);
		case 2: strmid(bData[bizid][bWorker2], pData[playerid][pName], 0, strlen(pData[playerid][pName]), 256);
		case 3: strmid(bData[bizid][bWorker3], pData[playerid][pName], 0, strlen(pData[playerid][pName]), 256);
		case 4: strmid(bData[bizid][bWorker4], pData[playerid][pName], 0, strlen(pData[playerid][pName]), 256);
		case 5: strmid(bData[bizid][bWorker5], pData[playerid][pName], 0, strlen(pData[playerid][pName]), 256);
	}
	return 1;
}

Resetbiz_Worker(id, bizid)
{
	switch (id) {
		case 1: format(bData[bizid][bWorker1], 24, "Vacant");
		case 2: format(bData[bizid][bWorker2], 24, "Vacant");
		case 3: format(bData[bizid][bWorker3], 24, "Vacant");
		case 4: format(bData[bizid][bWorker4], 24, "Vacant");
		case 5: format(bData[bizid][bWorker5], 24, "Vacant");
	}
	return 1;
}
...
Bisnis_Reset(id)
{
	...
	bData[id][bVisit] = 0;
	bData[id][bRestock] = 0;
	format(bData[id][bWorker1], MAX_PLAYER_NAME, "Vacant");
	format(bData[id][bWorker2], MAX_PLAYER_NAME, "Vacant");
	format(bData[id][bWorker3], MAX_PLAYER_NAME, "Vacant");
	format(bData[id][bWorker4], MAX_PLAYER_NAME, "Vacant");
	format(bData[id][bWorker5], MAX_PLAYER_NAME, "Vacant");
	bData[id][bWork] = 0;
	Bisnis_Refresh(id);
}
...
function LoadBisnis()
{
	...
	new worker1[128], worker2[128], worker3[128], worker4[128], worker5[128];
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name(i, "worker1", worker1);
			format(bData[bid][bWorker1], 128, worker1);
			cache_get_value_name(i, "worker2", worker2);
			format(bData[bid][bWorker2], 128, worker2);
			cache_get_value_name(i, "worker3", worker3);
			format(bData[bid][bWorker3], 128, worker3);
			cache_get_value_name(i, "worker4", worker4);
			format(bData[bid][bWorker4], 128, worker4);
			cache_get_value_name(i, "worker5", worker5);
			format(bData[bid][bWorker5], 128, worker5);
			cache_get_value_name_int(i, "work", bData[bid][bWork]);
			Bisnis_Refresh(bid);
			Iter_Add(Bisnis, bid);
		}
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}LC:RP {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
...
CMD:bm(playerid, params[])
{
	...
	} else return Error(playerid,"You are in own business.");
    return 1;
}
...
CMD:inviteworker(playerid, params[])
{
	new otherid, worker, id = pData[playerid][pInBiz];

	if (sscanf(params, "ud", otherid, worker))
		return Usage(playerid, "/inviteworker [playerid/name] [worker 1-5]");

	if (!IsPlayerConnected(otherid)) return Error(playerid,"Invalid playerid!");
	if (!Player_OwnsBisnis(playerid, pData[playerid][pInBiz])) return Error(playerid, "You must in your business!");
	...
	if (worker < 1 || worker > 5) return Error(playerid,"Invalid worker!");
	if (bData[id][bWork] >= 5) return Error(playerid,"You business employee is full!");

	pData[otherid][pWorkerBiz] = pData[playerid][pInBiz];
	Setbiz_Worker(id, worker, otherid);
	bData[id][bWork]++;
...
CMD:uninviteworker(playerid,params[])
{
	new otherid, worker, id = pData[playerid][pInBiz];

	if (sscanf(params, "ud", otherid, worker))
		return Usage(playerid, "/uninviteworker [playerid/name] [worker 1-5]");
	...
	Resetbiz_Worker(worker, id);
	bData[id][bWork]--;
	Info(playerid,"You has been fired %s to your business.", pData[otherid][pName]);
	Info(otherid,"%s has fired your to business.", pData[playerid][pName]);
	Bisnis_Save(id);
	return 1;
}