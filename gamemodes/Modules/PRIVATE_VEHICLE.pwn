...
function RemovePlayerVehicle(playerid)
{
	foreach(new i : PVehicles)
	{
		...
		{
			new cQuery[3048]/*, color1, color2, paintjob*/;
			pvData[i][cOwner] = -1;
...
function OnVehBuyPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a)
{
	...
	GetVehicleDamageStatus(i,panels,doors,light,tires);
	UpdateVehicleDamageStatus(i,0,0,0,tires);
	for(new j = 0; j < 17; j++)
		pvData[i][cMod][j] = 0;
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	...
	// SetPlayerPosition(playerid, 1294.0907,-1439.6799,13.6289, 177.54, 0);
...
function OnVehBuyVIPPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a)
{
	...
	SetPlayerPosition(playerid, 1294.0907,-1439.6799,13.6289, 177.45, 0);
	return 1;
}
...
function OnVehRentPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a, rental)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Anda telah menyewa kendaraan dengan model %s(%d)", GetVehicleModelName(model), model);
	// SetPlayerPosition(playerid, 1297.1675,-1439.6792,13.6289, 177.45, 0);
	pData[playerid][pBuyPvModel] = 0;
	return 1;
}
    ...
    else if(!strcmp(type,"trunk",true))
    {
        ...
        switch (GetTrunkStatus(vehicleid))
        {
            case false:
            {
                ...
                // new str[256];
                // format(str, sizeof(str), "Items\tAmount\n");
                // format(str, sizeof(str), "%sComponent\t%d\nMaterials\t%d\n%s\t%d\nAdd Items", ...)
                // ShowPlayerDialog(playerid, DIALOG_TRUNK, DIALOG_STYLE_LIST, "Vehicle Trunk", str, "Select", "Close");
            }
            case true:
            {
...
{		
	new vehid;
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1297.1675,-1439.6792,13.6289)) return Error(playerid, "You must in showroom/dealer!");
